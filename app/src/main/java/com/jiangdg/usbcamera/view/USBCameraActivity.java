...
import com.jiangdg.usbcamera.MCManager;
...
public class USBCameraActivity extends AppCompatActivity implements CameraDialog.CameraDialogParent{
     ...
     * USB设备事件监听器
     * */
    private MCManager.OnMyDevConnectListener listener = new MCManager.OnMyDevConnectListener() {
        // 插入USB设备
        @Override
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // 初始化引擎
        mUSBManager = MCManager.getInstance();
        ...
        switch (vId) {
            ...
            case R.id.btn_contrast:
                if(mUSBManager == null || !mUSBManager.isCameraOpened())
                    return;
                int contrast = mUSBManager.getModelValue(MCManager.MODE_CONTRAST);
                mUSBManager.setModelValue(MCManager.MODE_CONTRAST,contrast++);
                break;
            // 亮度
            case R.id.btn_brightness:
                if(mUSBManager == null || !mUSBManager.isCameraOpened())
                    return;
                int brightness = mUSBManager.getModelValue(MCManager.MODE_BRIGHTNESS);
                mUSBManager.setModelValue(MCManager.MODE_BRIGHTNESS,brightness++);
                break;
            // 重启Camera
            ...
            case R.id.btn_update_resolution:
                if(mUSBManager == null || !mUSBManager.isCameraOpened())
                    return;
                mUSBManager.updateResolution(320, 240, new MCManager.OnPreviewListener() {
                    @Override
...
//                showShortMsg("对焦相机");
            ...
            case R.id.btn_capture_pic:
                ...
                String picPath = MCManager.ROOT_PATH+System.currentTimeMillis()
                        + MCManager.SUFFIX_PNG;
            ...
            case R.id.btn_rec_video:
                ...
                if(! mUSBManager.isRecording()){
                    String videoPath = MCManager.ROOT_PATH+System.currentTimeMillis();
